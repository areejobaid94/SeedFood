...
<div >
                ...
                >
                  <div
                    v-for="(subcategory,i) of this.selectedCategorySub.subCategorysInItemModels"
                    :key="i"
                    class="nav-item nav-link"
                    :class="{'active':selectedCategorySub.categoryId===subcategory.categoryId}"
...
export default {
  ...
  methods: {
    ...
    getAllCategory () {
     ...
     axios.get(`https://infofoodservices.azurewebsites.net/api/MenuSystem/GetMenuItem?TenantID=${this.tenantID}&MenuType=${this.menuType}&LanguageBotId=${this.languageBot}`, {
        headers: {
          'Content-Type': 'application/json',
          'Access-Control-Allow-Origin': '*'
        }
