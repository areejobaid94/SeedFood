<template>
  <div v-if="item" class="product-details">
    <form method="post" @submit="checkOut">
      ...
      <div class="container d-flex flex-row justify-content-center checkout-btn cart-btn" >
...
if(this.tenantID !=34){
this.getItemDetailsById(this.item.id)
}
      else{
...
debugger
      if((this.tenantID==34 || this.item.isQuantitative == true )&& this.item.viewPrice==0){
...
debugger
    ...
    getItemById(id) {
     ...
     getItemDetailsById(id) {
      axios.get(`${this.$axios.defaults.baseURL}/api/MenuSystem/GetItemById?id=${id}&tenantId=${this.tenantID}`, {
        headers: {
          'Content-Type': 'application/json',
          'Access-Control-Allow-Origin': '*'
        }
        ...
        this.item = res.data.result
      if(this.tenantID !=34 && this.item.oldPrice!=null ){

 var oprice = this.item.oldPrice;
  var price = this.item.price;

  this.item.oldPrice =price;
  this.item.price =oprice;
      }
 if (this.item.itemAdditionDtos) {
        this.item.itemAdditionDtos.forEach((item2) => {
          this.selectedExtraChoices.push({
