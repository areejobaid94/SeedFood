...
using CampaignSyncFunction.WhatsAppApi;
using CampaignSyncFunction.WhatsAppApi.Dto;
...
{
    public static class SendCampaign
    {

        [FunctionName("CampaignSyncFunction")]
        public static void Run([QueueTrigger("campaign-sync", Connection = "AzureWebJobsStorage")] string message, ILogger log)
        {



            //Run([QueueTrigger("campaign-sync", Connection = "AzureWebJobsStorage")] string message)
            //Run([TimerTrigger("0 1 * * * *", RunOnStartup = true)] TimerInfo myTimer)
            // log.LogInformation($"C# Queue trigger function processed: {message}");
            //WhatsAppFunModel obj = new WhatsAppFunModel();
            //WhatsAppContactsDto whatsAppContactsDto = new WhatsAppContactsDto();
            //whatsAppContactsDto.PhoneNumber = "962786464718";
            //obj.whatsAppContactsDto =
            //obj.campaignId = 677;
            //obj.templateId = 665;
            //obj.UserId = "27_962786464718";
            //obj.TenantId = 27;
        ...
        public static async Task Sync(WhatsAppFunModel model, ILogger log)
        {
            try
            {
                ...
                MessageTemplateModel templateWA = getTemplateByWhatsAppId(log, tenant, objWhatsAppTemplateModel.id).Result;

                model.DailyLimit = getDailyLimitByTenantId(model.TenantId);
                //model.DailyLimit= getTenantDailyLimit(model.TenantId);
                ...
                if (templateWA != null && templateWA.status == "APPROVED")
                {
                    ...
                    await NewSendCampaignFunAsync(log, model, Statistics, model.IsContact, tenant, objWhatsAppTemplateModel, pageNumber);
                    ...
                    whatsAppCampaignModel = new WhatsAppCampaignModel
                    {
                        id = model.campaignId,
                        sentTime = DateTime.UtcNow,
                        status = (int)WhatsAppCampaignStatusEnum.Sent,  // end 
                        SentCampaignId = model.GuidId
                    };
                    updateWhatsAppCampaign(log, whatsAppCampaignModel);
                }
        ...
        private static async Task NewSendCampaignFunAsync(ILogger log, WhatsAppFunModel model, GetAllDashboard Statistics, bool IsContacts, TenantModel tenant, MessageTemplateModel objWhatsAppTemplateModel, int pageNumber)
        {
            try
            {
                //try
                ...
                if (IsContacts)
                {
                    model.whatsAppContactsDto.TemplateId = model.templateId;
                    model.whatsAppContactsDto.CampaignId = model.campaignId;
                    model.whatsAppContactsDto.pageNumber = pageNumber;
                    model.whatsAppContactsDto.pageSize = pageSize;
                    contactsEntity = getFilterContacts(log, model.whatsAppContactsDto, model.TenantId);
                }
                else
                {
                    contactsEntity = getExternalContacts(log, model.campaignId, model.templateId, pageNumber, pageSize, model.TenantId);
                }



                if ((contactsEntity.TotalCount - contactsEntity.TotalOptOut) > 0 && contactsEntity != null && model.DailyLimit >= contactsEntity.TotalCount)
                {
                    ...
                    foreach (var contact in contactsEntity.contacts)
                    {
                        ...
                        {
                            contact.templateVariables.VarOne=model.Parameters;
                        }

                        // CustomerOPT :  0=> Neutral , 1=> OPT-Out , 2=> OPT-In       
                        if (contact.CustomerOPT != 1)
                        {
                            ...
                            {
                                var contactCampaignModel = await NewSendTemplateToWhatsAppFun(log, tenant, postBody, model.campaignId, _postMessageTemplateModel.to, contact.Id, IsContacts, model.msg, model.type, objWhatsAppTemplateModel.mediaLink, model.TenantId, model.UserId, model.templateId, LstSendCampaignFailedModel, model.GuidId, lstcontactCampaignModel, objWhatsAppTemplateModel.id, IsSent);
                    ...
                    if (totalCost != 0)
                    {
                        ...
                        usersDashModel = GetUserInfo(long.Parse(model.UserId));
                        ...
                        usageDetailsModel.campaignId = CampaignId;
                    ...
                    if (lstcontactCampaignModel != null && lstcontactCampaignModel.Count > 0) // add list Contact
                    {
                        decimal UsageBIRate = 0, UsageFreeRate = 0;
                        foreach (var row in lstcontactCampaignModel)
                        {
                            ...
                            else if (Statistics.RemainingFreeConversation >= 1 && Statistics.RemainingBIConversation < 1)
                            {
                                UsageFreeRate += 1;
                                Statistics.RemainingFreeConversation -= 1;
                            }
                        }
                        NewaddContactCampaignFun(lstcontactCampaignModel);
                        UpdateBIConversation(model.TenantId, UsageBIRate, UsageFreeRate);

                    }

                    if (LstSendCampaignFailedModel != null && LstSendCampaignFailedModel.Count > 0) ///failed list
                    {
                    ...
                    if ((contactsEntity.TotalCount - contactsEntity.TotalOptOut) > 0 && contactsEntity.contacts.Count > 0 && num != 0)
                    {
                        pageNumber++;
                        await NewSendCampaignFunAsync(log, model, Statistics, IsContacts, tenant, objWhatsAppTemplateModel, pageNumber);
                    }
                    //if ((contactsEntity.TotalCount - contactsEntity.TotalOptOut) > 0 && contactsEntity.contacts.Count > 0)
                    //{
                    //    pageNumber++;
                    //    await NewSendCampaignFunAsync(log, model, Statistics, IsContacts, tenant, objWhatsAppTemplateModel, pageNumber);
                    //}
        ...
        private static async Task SendCampaignAsync(ILogger log, WhatsAppFunModel model, GetAllDashboard Statistics, bool IsContacts, TenantModel tenant, MessageTemplateModel objWhatsAppTemplateModel)
        {
            try
            {
                ContactsEntity contactsEntity = new ContactsEntity();
                ...
                if ((contactsEntity.TotalCount - contactsEntity.TotalOptOut) > 0 && contactsEntity != null && getDailyLimitByTenantId(model.TenantId) >= contactsEntity.TotalCount)
                {
                    ...
                    foreach (var contact in contactsEntity.contacts)
                    {
                        ...
                        if (contact.CustomerOPT != 1)
                        {
                            PostMessageTemplateModel _postMessageTemplateModel = prepareMessageTemplate(objWhatsAppTemplateModel, contact);
                            var postBody = JsonConvert.SerializeObject(_postMessageTemplateModel, Formatting.None, new JsonSerializerSettings { NullValueHandling = NullValueHandling.Ignore });
                            tbl = await SendTemplateToWhatsApp(log, tenant, postBody, model.campaignId, _postMessageTemplateModel.to, contact.Id, tbl, IsContacts, msg, type, objWhatsAppTemplateModel.mediaLink, model.TenantId, model.UserId, model.templateId, LstSendCampaignFailedModel, model.GuidId);
                        }
                    }
                    ...
                    if (tbl.Rows.Count > 0)
                    {
                        ...
                        foreach (DataRow row in tbl.Rows)
                        {
                            ...
                            //}
                            //else if (Statistics.RemainingFreeConversation >= countryCode.Rate && Statistics.RemainingBIConversation < countryCode.Rate)
                            //{
                            //    UsageFreeRate += 1;
                            //    Statistics.RemainingFreeConversation -= 1;
                            //}
                    ...
                    if ((contactsEntity.TotalCount - contactsEntity.TotalOptOut) > 0)
                    {
                        // Thread.Sleep(2000);
                        await SendCampaignAsync(log, model, Statistics, IsContacts, tenant, objWhatsAppTemplateModel);
                    }
                    WhatsAppCampaignModel whatsAppCampaignModel = new WhatsAppCampaignModel
                    {
                        id = model.campaignId,
        ...
        private static async Task<ContactCampaignModel> NewSendTemplateToWhatsAppFun(ILogger log, TenantModel tenant, string postBody, long campaignId, string phoneNumber, int? ContactId, bool IsContacts, string msg, string type, string mediaUrl, int TenantId, string UserId, long templateId, List<SendCampaignFailedModel> lstSendCampaignFailedModel, Guid guid, List<ContactCampaignModel> lstcontactCampaignModel, string WTID, bool IsSent)
        {
            try
            {
                ...
                using (var httpClient = new HttpClient())
                {
                    ...
                    using (var response = await httpClient.PostAsync(postUrl, new StringContent(postBody, Encoding.UTF8, "application/json")))
                    {
                        ...
                        if (response.StatusCode == System.Net.HttpStatusCode.OK)
                        {
                            ...
                            contactCampaignModel.MessageRate = 1;//getCoutryBIRate(phoneNumber).Rate;
                            contactCampaignModel.IsSent = true;


                            //if (IsContacts)
                            //{
                            //    await UpdateCustomerChatAsync(phoneNumber, msg, type, mediaUrl, TenantId, UserId, templateResult.messages.FirstOrDefault().id, templateId);
                            //}
        ...
        private static async Task<DataTable> SendTemplateToWhatsApp(ILogger log, TenantModel tenant, string postBody, long campaignId, string phoneNumber, int? ContactId, DataTable tbl, bool IsContacts, string msg, string type, string mediaUrl, int TenantId, string UserId, long templateId, List<SendCampaignFailedModel> lstSendCampaignFailedModel, Guid guid)
        {
            try
            {
                ...
                using (var httpClient = new HttpClient())
                {
                    ...
                    using (var response = await httpClient.PostAsync(postUrl, new StringContent(postBody, Encoding.UTF8, "application/json")))
                    {
                        ...
                        if (response.StatusCode == System.Net.HttpStatusCode.OK)
                        {
                            ...
                            dr["MessageRate"] = 1;//getCoutryBIRate(phoneNumber).Rate;
        ...
        private static async Task UpdateCustomerChatAsync(string phoneNumber, string msg, string type, string mediaUrl, int TenantId, string UserId, string massageId = "", long templateId = 0)
        {
            string userId = TenantId + "_" + phoneNumber;
            CustomerChat customerChat = new CustomerChat()
            {
        ...
        private static ContactsEntity getExternalContacts(ILogger log, long campaignId, long templateId, int pageNumber, int pageSize, int? tenantId = null)
        {
            try
            {
                ContactsEntity contactsEntity = new ContactsEntity();
