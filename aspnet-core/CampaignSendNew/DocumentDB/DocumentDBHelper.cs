namespace CampaignSendNew
{
    ...
    public class DocumentDBHelper<T> where T : class
    {
        ...
        public async Task<T> BulkInsertDocumentsAsync(List<T> items)
        {
            try
            {
                // Assume 'client' and 'DocumentCollection' are defined elsewhere in your code
                var storedProcedureUri = UriFactory.CreateStoredProcedureUri(Constants.Database, DocumentCollection, "bulkInsert");

                var result = await client.ExecuteStoredProcedureAsync<int>(storedProcedureUri, new RequestOptions(), items);
        ...
        public async Task<List<DocumentResult>> CreateItemsAsync(List<T> items)
        {
