...
using Infoseed.MessagingPortal.Test.Base.TestData;
...
namespace Infoseed.MessagingPortal.Test.Base
{
    ...
    /// Provides methods to easily work with <see cref="MessagingPortalDbContext"/>.
    /// </summary>
    public abstract class AppTestBase<T> : AbpIntegratedTestBase<T> where T : AbpModule
    {
        ...
        {
            void NormalizeDbContext(MessagingPortalDbContext context)
            {
        ...
        protected void UsingDbContext(Action<MessagingPortalDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<MessagingPortalDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected TResult UsingDbContext<TResult>(Func<MessagingPortalDbContext, TResult> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<TResult> UsingDbContextAsync<TResult>(Func<MessagingPortalDbContext, Task<TResult>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<MessagingPortalDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MessagingPortalDbContext>())
                {
                    action(context);
                    context.SaveChanges();
                }
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<MessagingPortalDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<MessagingPortalDbContext>())
        ...
        protected TResult UsingDbContext<TResult>(int? tenantId, Func<MessagingPortalDbContext, TResult> func)
        {
            TResult result;
        ...
        protected async Task<TResult> UsingDbContextAsync<TResult>(int? tenantId, Func<MessagingPortalDbContext, Task<TResult>> func)
        {
            TResult result;
