...
using Infoseed.MessagingPortal.Web.Authentication.JwtBearer;
using Infoseed.MessagingPortal.Web.Authentication.TwoFactor;
using Infoseed.MessagingPortal.Web.Models.TokenAuth;
...
using Infoseed.MessagingPortal.Net.Sms;
using Infoseed.MessagingPortal.Notifications;
using Infoseed.MessagingPortal.Security.Recaptcha;
using Infoseed.MessagingPortal.Web.Authentication.External;
...
namespace Infoseed.MessagingPortal.Web.Controllers
{
    ...
    public class TokenAuthController : MessagingPortalControllerBase
    {
        ...
        public async Task<AuthenticateResultModel> Authenticate([FromBody] AuthenticateModel model)
        {
            ...
            //loginResult.Identity.AddClaim(new Claim("http://www.aspnetboilerplate.com/identity/claims/timezone", "Jordan Standard Time"));
            //loginResult.Identity.AddClaim(new Claim("http://www.aspnetboilerplate.com/identity/claims/currencycode", "JOD"));
            ...
            {
                var isIpAddresess = _userAppService.CheckIpAddress(AbpSession.TenantId.Value, int.Parse(loginResult.User.Id.ToString()));
        ...
        [HttpPost]
        public async Task<AuthenticateResultModel> AuthenticatePhone2([FromBody] AuthenticateModel model)
        {
            if (UseCaptchaOnLogin())
            {
                await ValidateReCaptcha(model.CaptchaResponse);
        ...
        [HttpPost]
        public async Task<AuthenticateResultModel> AuthenticatePhone([FromBody] AuthenticateModel model)
        {
            if (UseCaptchaOnLogin())
            {
                await ValidateReCaptcha(model.CaptchaResponse);
