...
namespace Infoseed.MessagingPortal.Authentication.TwoFactor.Google
{
    public class GoogleAuthenticatorProvider : MessagingPortalServiceBase, IUserTwoFactorTokenProvider<User>, ITransientDependency
    {
        ...
        public Task<string> GenerateAsync(string purpose, UserManager<User> userManager, User user)
        {
            ...
            var setupInfo = _googleTwoFactorAuthenticateService.GenerateSetupCode("Infoseed.MessagingPortal", user.EmailAddress, user.GoogleAuthenticatorKey, 300, 300);
