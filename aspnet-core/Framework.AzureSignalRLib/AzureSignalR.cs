...
namespace Framework.AzureSignalRLib
{
   public class AzureSignalR
    {
        ...
        public async Task SendAsync(string hubName,string userName, string methodName, params object[] args)
        {
            ...
            {
                url = $"{endpoint}/api/v1/hubs/{hubName}/users/{userName}";
            }
        ...
        public async Task SendAsync(Hubs hubName, string userName, Methods methodName, params object[] args)
        {
            var payload = new PayloadMessage()
            {
                Target = methodName.ToString(),
                Arguments = args
            };
            string url = string.Empty;
            if (string.IsNullOrWhiteSpace(userName))
