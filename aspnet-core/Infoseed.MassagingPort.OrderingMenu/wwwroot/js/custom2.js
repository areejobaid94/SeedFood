...
$(document).on("click", ".btn-filter", function (e) {
    ...
    if ($target != 'all') {
        ...
        setTimeout(function () {
            $('html, body').animate({
                //Move the owlCarousel after clicking on the subcategory
...
function filterSelection(c, sublist) {
    ...
    objItems = replaceArabicDecimalSeparator(objItems);
...
$(document).on("click", ".plusNew", function () {
    ...
    var inputElement = document.getElementById("txtItemQty");
    var value = parseInt(inputElement.value);

    // Check if the value is less than 1 and set it to 1 if so
    if (value < 1) {
        inputElement.value = 1;
    }

    // Check if the value is greater than 999 and set it to 999 if so
    if (value > 999) {
        inputElement.value = 999;
    }
    return false;
});
$(document).on("click", ".plusNewAdditions", function () {
    ...
    var inputElements = document.getElementsByClassName("hdItemQtyAddtional");
    ...
    for (var i = 0; i < inputElements.length; i++) {
        ...
        if (value > 999) {
            inputElement.value = "999"; // Set it to 999
        }
    }
    return false;
});
$(document).on("click", ".plusLast", function () {
    ...
    var inputElements = document.getElementsByClassName("LastCard");
...
$(document).on("click", "#btnAddToCart", function (index, e) {
    ...
    if (isValidItem($('#txtItemQty').val())) {
        ...
        objItem = replaceArabicDecimalSeparator(objItem);
...
function enforceMinMaxAdditions(inputElement) {

    if (/[^0-9]/.test(inputElement.value)) {
        // If there are non-numeric characters, display an error message
        nputElement.value = inputElement.value.replace(/[^0-9]/g, ''); // Remove non-numeric characters
    } else {
...
function enforceMinMaxLast(inputElement) {

    var value = parseInt(inputElement.value);

    var $input = $(inputElement); // Select the input element using a jQuery selector
    var inputValue = $input.val();
    ...
    var inputElements = document.getElementsByClassName("test");

    for (var i = 0; i < inputElements.length; i++) {
        inputElements[i].click(); // Trigger click event on each element
    }
    getItemSelectItem();
}
...
function createClickableLinks(input) {
    // Regular expression pattern to match URLs
    var pattern = /(http|https|ftp|ftps):\/\/[^\s/$.?#].[^\s]*/g;
    // Replace matched URLs with clickable links
    var content = input.replace(pattern, function (match) {
        return '<a href="' + match + '" target="_blank" style="color: #007ad9;">' + match + '</a>';
    });
    // Update the content of a specific HTML element with the clickable links
...
function replaceArabicDecimalSeparator(obj) {
    // Convert the object to a JSON string
    let jsonString = JSON.stringify(obj);

    // Replace Arabic decimal separator with English dot
        ...
        if (x == "True") {
            ...
            if (point <= 0 || point == null) {
                ...
                if (TotalLoyaltyPointsForBasket != null) {
                    ...
                    TotalLoyaltyPointsForBasket.style.display = 'block';
                    TotalLoyaltyPointsForBasket.style.opacity = 1;
                }

                if (TotalLoyaltyPoint != null) {
                    TotalLoyaltyPoint.disabled = true;
                    TotalLoyaltyPoint.style.display = 'none';
...
function fillDefaultScript() {
   ...
   // var lang = $(this).data('lang');
    ...
    $('.slider').owlCarousel({
        ...
        autoWidth: true,
        navSpeed: 100,// milliseconds
        smartSpeed: 100, // milliseconds
        dragEndSpeed: 100, // milliseconds
