...
namespace Infoseed.MessagingPortal.Sunshine.Infrastructure
{
    public static class SunshineConnector
    {
        ...
        public static async Task<HttpStatusCode> PostMsgToSmooch(string appID, string conversationID, Content msg)
        {
            ...
            var client = getSunshineHttpClient();
        ...
        public static HttpClient getSunshineHttpClient()
        {
            var client = new HttpClient();
            //Make the basic dynamic = Encode AppiD:SecretKey 
            string appID = ConfigurationManager.AppSettings["smoochAppID"]; 
            string secretKey = ConfigurationManager.AppSettings["smoochSecretKey"]; 
