...
import { EntityDtoOfInt64, TeamsDtoModel, TeamsServiceProxy, UserListDto, UserServiceProxy } from '@shared/service-proxies/service-proxies';
...
import { CreateOrEditUserSettingModalComponent } from './create-or-edit-user-setting-modal.component';
import { CreateOrEditBookingSettingsComponent } from './create-or-edit-booking-settings.component';
import * as rtlDetect from 'rtl-detect';
import 'moment-hijri'; // Hijri extension for moment.js
import moment from 'moment';
 import { CreateOrEditTeamsModalComponent } from './create-or-edit-teams-modal.component';
import { UserTicketsModalComponent } from './user-tickets-modal.component';
...
export class UsersComponent extends AppComponentBase implements AfterViewInit {
    ...
    @ViewChild("createOrEditTeamsModal", { static: false })
    createOrEditTeamsModal: CreateOrEditTeamsModalComponent;

    @ViewChild('userTicketsModal', { static: true }) userTicketsModal: UserTicketsModalComponent;
    ...
    @ViewChild('createOrEditUserSettingModel', { static: true }) createOrEditUserSettingModel: CreateOrEditUserSettingModalComponent;
    @ViewChild('createOrEditBookingSettings', { static: true }) createOrEditBookingSettings: CreateOrEditBookingSettingsComponent;
    ...
    getUsers(event?: LazyLoadEvent, tabIndex? :any) {
...
if(tabIndex==undefined){
    tabIndex=0;
          ...
          if(tabIndex==0){
            ...
            this._teamsService
            .teamsGetAll(this.primengTableHelper.getSorting(this.dataTable),
            this.primengTableHelper.getSkipCount(this.paginator, event),
            this.primengTableHelper.getMaxResultCount(this.paginator, event))
            .subscribe((res) => {
    
                this.primengTableHelper.totalRecordsCount = res.total;
                this.primengTableHelper.records = res.teamsDtoModel;
                this.setUsersProfilePictureUrl(this.primengTableHelper.records);
                this.primengTableHelper.hideLoadingIndicator();
    ...
    createUser(): void {
        this.createOrEditUserModal.show(0);
    }
    createTeams(): void {
        
         this.createOrEditTeamsModal.show();
    }
    editTeams(model:TeamsDtoModel): void {
        
        this.createOrEditTeamsModal.showEdit(model);
   }
    ...
    deleteTeams(user: TeamsDtoModel): void {


        this.message.confirm(
            this.l('UserDeleteWarningMessage', user.teamsName),
            this.l('AreYouSure'),
            (isConfirmed) => {
                if (isConfirmed) {
                    this._teamsService.teamsDelete(user.id)
                        .subscribe(() => {
                            this.reloadPage();
                            this.notify.success(this.l('successfullydeleted'));
                        });
    ...
    showDynamicProperties(user: UserListDto): void {
        this.dynamicPropertiesModal.show('Infoseed.MessagingPortal.Authorization.Users.User', user.id.toString());
    }
    ...
    switchHeaders(tabNumber: any) {
        ...
        if(tabNumber.index == 0){
            this.getUsers(null,tabNumber.index)
