...
import { LiveChatServiceProxy, TeamsServiceProxy } from "@shared/service-proxies/service-proxies";
    ...
    implements OnInit
{
    ...
    selectedUsersTeamsToAssign: assignedUser[] = [];
...
debugger
        ...
        if(type==1){
            ...
            let tempSelectedUsersTeamsIds;
            tempSelectedUsersTeamsIds = ticket.teamsIds.split(",");

            this.selectedUsersToAssign = [];
            this.unChangedUsersToAssign = [];


            this.selectedUsersTeamsToAssign=[]

            this._teamsService
            .teamsGetAll(null,0, 100000)
            .subscribe((result) => {
                debugger
             result.teamsDtoModel.forEach((user) => {
                debugger
                let temp: assignedUser = {
                    id: user.id,
                    fullName: user.teamsName,
                    isActive: false,
                };
                if (
                    tempSelectedUsersTeamsIds.findIndex(
                        (id) => parseInt(id, 10) === user.id
                    ) != -1
                ) {
                    temp.isActive = true;
                ...
                this.selectedUsersToAssign.push(temp);
                this.selectedUsersTeamsToAssign.push(temp);
    ...
    assignUsers() {
        let userIds: string = this.selectedUsersToAssign
            ...
            let teamsIds: string = this.selectedUsersTeamsToAssign
            .filter((item) => item.isActive) // Filter out items where `active` is true
            .map((item) => item.id) // Map to the `id` property
            .join(",");
    
        this._liveChatServiceProxy
            .assignLiveChatToUser(this.ticketId, userIds, undefined, this.appSession.user.userName, this.appSession.user.id,teamsIds)
            .subscribe(() => {
                this.toastr.info(
