...
get safeMediaLink(): SafeResourceUrl {
  ...
  return this.sanitizer.bypassSecurityTrustResourceUrl(this.templateById.mediaLink);
    ...
    test22(){
        ...
        debugger
        this.firstButtonURLVariabllesTemplate;
    ...
    try {
        ...
        // Additional pattern check if needed (from your original code)
    ...
    handleSend(Schedule: boolean = false) {
        ...
        // let cardVariables: CardVariabllesTemplate;
...
validateAllUrlVariables(): boolean {
  const cardIndexes = Object.keys(this.urlVariables);

  for (const cardIndex of cardIndexes) {
    const cardIdx = Number(cardIndex);
    const buttonIndexes = Object.keys(this.urlVariables[cardIdx] || {});

    for (const buttonIndex of buttonIndexes) {
      const btnIdx = Number(buttonIndex);
      const inputs = this.urlVariables[cardIdx][btnIdx];
        ...
        this._whatsAppMessageTemplateServiceProxy
            ...
            if (this.componentCarousel) {
                ...
                this.cards.forEach((card, cardIndex) => {
                    ...
                    // Step 3: Assign each button to a separate variable
                    ...
                    //this.inputCountFromBackend = this.templateById.variableCount;
                        // this.VariableCountHeader=this.templateById.variableHeaderCount;
