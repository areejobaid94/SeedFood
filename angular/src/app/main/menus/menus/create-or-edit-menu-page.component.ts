...
import {
    ...
    ItemsServiceProxy,
    CreateOrEditItemDto, ItemAdditionDto, RestaurantsTypeEunm, RType, LanguageBot, AdditionsCategorysListModel, CreateOrEditItemAndAdditionsCategoryDto, GetSpecificationsCategorysModel, ItemAdditionServiceProxy, ItemSpecificationsDto, TenantTypeEunm
...
@Component({
    selector: 'app-create-or-edit-menu-page',
    templateUrl: './create-or-edit-menu-page.component.html',
})
export class CreateOrEditMenuPageComponent extends AppComponentBase implements OnInit {
    theme: string;
    menuIdd: number;
    ...
    @ViewChild('imageCroppersModal', { static: true }) imageCrppersComponent: ImageCrppersComponent;
    ...
    openSelectAdditionsCategoryModal(cat, sub, ind, add,itemIndex) {
        this.cat = cat;
        this.ind = ind;
        this.sub = sub;
        this.add = add;
    ...
    openSelectSpecificationModal(cat, sub, ind, add,itemIndex) {
    ...
    setdditionsCategoryNull() {
        ...
        if (!this.isMall) {
            ...
            if (!this.addOnsLookupTableModal.isCloseClick) {
                ...
                const control = ((<UntypedFormArray>this.menuForm.controls['category']).at(this.cat).get('items') as UntypedFormArray).at(this.ind).get('additionsCategorysListModels') as UntypedFormArray;
            ...
            if (!this.addOnsLookupTableModal.isCloseClick) {
                ...
                const control = (((<UntypedFormArray>this.menuForm.controls['category']).at(this.cat).get('subcategory') as UntypedFormArray).at(this.sub).get('items') as UntypedFormArray).at(this.ind).get('additionsCategorysListModels') as UntypedFormArray;
    ...
    UpdateAdditionsCategoryId() {
        ...
        if (!this.isMall) {
            if (!this.addOnsLookupTableModal.isCloseClick) {
                ...
                const control = ((<UntypedFormArray>this.menuForm.controls['category']).at(this.cat).get('items') as UntypedFormArray).at(this.itemIndex).get('additionsCategorysListModels') as UntypedFormArray;

                let item = control.at(this.add).value;
                item.AdditionsAndItemId = this.addOnsLookupTableModal.AdditionsCategorysid;
    ...
    UpdateSpecificationId() {
        ...
        if (!this.isMall) {
            if (!this.specificationLookupTableModal.isCloseClick) {
                ...
                const control = ((<UntypedFormArray>this.menuForm.controls['category']).at(this.cat).get('items') as UntypedFormArray).at(this.itemIndex).get('SpecificationModels') as UntypedFormArray;

                let item = control.at(this.add).value;
                //  item.AdditionsAndItemId = this.specificationLookupTableModal.AdditionsCategorysid;
                ...
                //  item.itemSpecificationId = this.specificationLookupTableModal.itemSpecificationId;
            ...
            if (!this.specificationLookupTableModal.isCloseClick) {
                ...
                const control = (((<UntypedFormArray>this.menuForm.controls['category']).at(this.cat).get('subcategory') as UntypedFormArray).at(this.sub).get('items') as UntypedFormArray).at(this.ind).get('SpecificationModels') as UntypedFormArray;
    ...
    show(menuId?: number): void {
        ...
        this._menusServiceProxy.getAdditionsCategorysList().subscribe(result => {

            this.additionsCategorysListModel = result;
            // this.languageBot.shift();
        });

        this._menusServiceProxy.getSpecificationsCategorys().subscribe(result => {
        ...
        //const sds = this.selectedCategoryType;
        // this.Rtype.push("Nafisa");
        // this.Rtype.push("FruitSalad");
        ...
        if (!menuId) {
            ...
            this._menusServiceProxy.getCategoryMenu(menuId).subscribe(res => {
                res.result.forEach(category => {
                    ...
                    categoryList.push(this.bindCategory(category.categoryId, category.categoryName, category.categoryNameEnglish, category.listItemInCategories));
                });
    ...
    async ngOnInit() {
        ...
        if (this.activeRoute.snapshot.queryParams['id'] != null) {
            this.menuIdd = this.activeRoute.snapshot.queryParams['id'];
            this.show(this.menuIdd);
            this.initCategory();
            this.initItem();
            this.initAddition();
            this.initItemAddition();
    ...
    initSpecificationListSet(item) {
        ...
        return this.fb.group({
            ...
            'specificationDescriptionEnglish': [item.nameEnglish, [Validators.required]],
            // ---------------------------------------------------------------------          
        });
    }
    ...
    bindCategory(id, name, nameEnglish, items) {
        ...
        if(items!=null || items!=undefined ){
            items.forEach(item => {
                ...
                data.push(this.bindItem(item.itemName, item.itemNameEnglish, item.price, item.priority, item.languageBotId, item.itemDescription, item.itemDescriptionEnglish, item.additionsCategorysListModels, item.itemSpecifications, item.imageUri, this.menuId, id, item.id, item.sku, item.isInService, item.oldPrice, item.ingredients));
            });
        }
    ...
    bindinitSpecification(id, isMultipleSelection, isRequired, priority, specificationDescription, specificationDescriptionEnglish, itemSpecificationId) {
        return this.fb.group({
            //  ---------------------forms fields on y level ------------------------
            'id': id,
            ...
            'specificationDescription': [specificationDescription, [Validators.required]],
            'specificationDescriptionEnglish': [specificationDescriptionEnglish, [Validators.required]],
        });
    }
    ...
    bindItem(name, nameEnglish, price, priority, languageBotId, description, descriptionEnglish, itemAddition, specification, image, menuId, categoryId, itemId, sKU, isInService, oldPrice, ingredients) {

        let data = [];
        let data2 = [];
        itemAddition.forEach(item => {

            data.push(this.bindinitAddition(item.id, item.name, item.nameEnglish, item.additionsAndItemId));
        });

        specification.forEach(item => {

            data2.push(this.bindinitSpecification(item.id, item.isMultipleSelection, item.isRequired, item.priority, item.specificationDescription, item.specificationDescriptionEnglish, item.itemSpecificationId));
        });
    ...
    addSubCategory(category) {
        ...
        const control = ((<UntypedFormArray>this.menuForm.controls['category']) as UntypedFormArray).at(category).get('subcategory') as UntypedFormArray;
    ...
    addItem(category, ix) {
        ...
        if (!this.isMall) {
            const control = (<UntypedFormArray>this.menuForm.controls['category']).at(ix).get('items') as UntypedFormArray;
            control.push(this.initItem());
        } else {
            const control = ((<UntypedFormArray>this.menuForm.controls['category']).at(category).get('subcategory') as UntypedFormArray).at(ix).get('items') as UntypedFormArray;
            control.push(this.initItem());
        }
    ...
    addItemAddition(category, subcategory, index) {
        ...
        if (!this.isMall) {
            const control = ((<UntypedFormArray>this.menuForm.controls['category']).at(subcategory).get('items') as UntypedFormArray).at(index).get('additionsCategorysListModels') as UntypedFormArray;
            control.push(this.initAdditionsCategoryList());
            this.openSelectAdditionsCategoryModal(category, 0, subcategory, control.length - 1,index);
            return;
        } else {
            const control = (((<UntypedFormArray>this.menuForm.controls['category']).at(category).get('subcategory') as UntypedFormArray).at(subcategory).get('items') as UntypedFormArray).at(index).get('additionsCategorysListModels') as UntypedFormArray;
            control.push(this.initAdditionsCategoryList());
        }
    ...
    addSpecification(category, subcategory, index) {
        ...
        if (!this.isMall) {
            const control = ((<UntypedFormArray>this.menuForm.controls['category']).at(subcategory).get('items') as UntypedFormArray).at(index).get('SpecificationModels') as UntypedFormArray;

            control.push(this.initSpecification());
            this.openSelectSpecificationModal(category, 0, subcategory, control.length - 1,index);
            return;
        } else {
            const control = (((<UntypedFormArray>this.menuForm.controls['category']).at(category).get('subcategory') as UntypedFormArray).at(subcategory).get('items') as UntypedFormArray).at(index).get('SpecificationModels') as UntypedFormArray;

            control.push(this.initSpecification());
        }
        //this.openSelectSpecificationModal(category,0, subcategory,control);
    ...
    deleteItem(category, subcategory, index) {
        ...
        if (!this.isMall) {
            ...
            const control = ((<UntypedFormArray>this.menuForm.controls['category']).at(category).get('subcategory') as UntypedFormArray).at(subcategory).get('items') as UntypedFormArray;
    ...
    deleteitemAddition(category, subcategory, index, add) {
        ...
        if (!this.isMall) {
            const control = ((<UntypedFormArray>this.menuForm.controls['category']).at(category).get('items') as UntypedFormArray).at(index).get('additionsCategorysListModels') as UntypedFormArray;
            ...
            if (item.id) {
                this.message.confirm(
                    ...
                    (isConfirmed) => {
                        if (isConfirmed) {
                            this._itemsServiceProxy.deleteItemAndAdditionsCategorys(item.additionsAndItemId)
                                .subscribe(() => {
                                    this.notify.success(this.l('successfullyDeleted'));
    ...
    deleteSpecification(category, subcategory, index, add) {
        ...
        if (!this.isMall) {
            const control = ((<UntypedFormArray>this.menuForm.controls['category']).at(category).get('items') as UntypedFormArray).at(index).get('SpecificationModels') as UntypedFormArray;
            ...
            if (item.itemSpecificationId) {
                this.message.confirm(
                    ...
                    (isConfirmed) => {
                        if (isConfirmed) {
                            this._itemAdditionServiceProxy.deleteItemSpecifications(item.itemSpecificationId)
                                .subscribe(() => {
                                    this.notify.success(this.l('successfullyDeleted'));
                                    control.removeAt(add);
    ...
    deleteCategoryItem(index) {
        ...
        if (item.id) {
            this.message.confirm(
                ...
                (isConfirmed) => {
                    if (isConfirmed) {
                        this._menuCategoriesServiceProxy.delete(item.id)
                            .subscribe(() => {
                                this.notify.success(this.l('successfullyDeleted'));
                                control.removeAt(index);
    ...
    deleteCategorySubItem(index, Subcatg) {
        ...
        const control = (<UntypedFormArray>this.menuForm.controls['category']).at(index).get('subcategory') as UntypedFormArray;
        ...
        //         (isConfirmed) => {
        //             if (isConfirmed) {
        //                 this._menuCategoriesServiceProxy.delete(cat.id)
        //                     .subscribe(() => {
        //                         this.notify.success(this.l('SuccessfullyDeleted'));
        //                         cat.removeAt(Subcatg);
    ...
    addMenu() {
        ...
        this._menusServiceProxy.createOrEdit(menu).subscribe(response => {
    ...
    addOrSaveCategory(menuId) {
        ...
        for (let i = 0; i < data.category.length; i++) {
            ...
            category.menuId= menuId !=0?menuId:this.menuId;
                       ...
                       // item.IsOverrideLoyaltyPoints=false;
                        ...
                        this._itemsServiceProxy.createOrEdit(item)
                            ...
                            .subscribe((res) => {
                                ...
                                obj.additionsCategorysListModels.forEach(element => {

                                    addAnditem.id = 0;
                                    addAnditem.itemId = item.id;
                                    addAnditem.menuType = this.menuType;;
                                    addAnditem.specificationId = 0;
                                    addAnditem.additionsCategorysId = element.id;
                                    addAnditem.additionsAndItemId = element.additionsAndItemId;
                                    
                                    this._menusServiceProxy.createOrEditItemAndAdditionsCategorys(addAnditem).subscribe(response => {
                                    });
        
                                });
        
        
                                let addAnditem2 = new ItemSpecificationsDto();
                                obj.SpecificationModels.forEach(element => {
        
                                    addAnditem2.id = element.itemSpecificationId;
                                    addAnditem2.itemId = item.id;
                                    addAnditem2.specificationId = element.id;
                                    addAnditem2.isRequired = element.isRequired;
                                    this._itemAdditionServiceProxy.createOrEditItemSpecifications(addAnditem2).subscribe(response => {
                                    });
